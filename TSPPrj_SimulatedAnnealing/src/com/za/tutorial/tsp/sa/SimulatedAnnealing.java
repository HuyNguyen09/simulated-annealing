package com.za.tutorial.tsp.sa;
...
public class SimulatedAnnealing {
	...
	public Route findRoute(double temperature, Route currentRoute) {
		...
		while (temperature > MIN_TEMPERATURE) {
			...
			if (acceptRoute(currentRoute.getTotalDistance(), adjacentRoute.getTotalDistance(), temperature)) currentRoute = new Route(adjacentRoute);
	...
	private boolean acceptRoute(double currentDistance, double adjacentDistance, double temperature) {
		...
		System.out.println(" | "+ String.format("%.2f", acceptanceProbability) + " |   " + String.format("%.2f", randomNumb) + "   | " + decision);
